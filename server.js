...
import { isUserOrAdmin } from './src/middlewares/roles.middleware.js';
...
import { productSchema } from './src/graphql/products.schema.js';
...
if(cluster.isPrimary){
    ...
    const io = new Server(server); //Crear canal.
    ...
    /*ROUTER PRODUCTOS GRAPHQL*/
    app.use('/graphql', graphqlHTTP({
        schema: productSchema,
        rootValue: {
            getAllProducts: productsDAO.getAll(),
            getProductByID: (id) => productsDAO.getById(id),
            createProduct: (product) => productsDAO.save(product),
            updateProductByID: (id, newData) => productsDAO.updateById(id, newData),
            deleteProductByID: (id) => productsDAO.deleteById(id)
        },
        graphiql: true 
    }));
    ...
    /*RUTA FAKER PRODUCTS*/
    app.get('/api/productos-test', [loggerRoute], [isUserOrAdmin], getFakerProducts);
