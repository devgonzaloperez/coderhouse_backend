    ...
    async getById(id){
        try{
            ...
            if(itemSelectedByIdFromDBParsed[0]){
                ...
                return productsDTO(itemSelectedByIdFromDBParsed)
            }
            else{
    ...
    async getAll(){
        try{
            ...
            if(itemsFromDBParsed && itemsFromDBParsed.length > 0){
                const allProductsDTO = itemsFromDBParsed.map((product) => productsDTO(product)); //En este caso, hay que mapearlo porque es un array y cada elemento debe ser pasado por el DTO.
